...
function ctlWC_cron_job_worker ($hardcron=false)
{
  ...
  $funds_received_value_expires_in_secs = $ctlwc_settings['funds_received_value_expires_in_mins'] * 60;
  $assigned_address_expires_in_secs     = $ctlwc_settings['assigned_address_expires_in_mins'] * 60;
  $confirmations_required = $ctlwc_settings['confs_num'];
  ...
  $query =
    "SELECT * FROM `$ctl_payments_table_name`
      WHERE
      (
  ...
  if (is_array($rows_for_balance_check))
  {
  	...
  	foreach ($rows_for_balance_check as $row_for_balance_check)
  	{
		  ...
		  $address_meta = ctlWC_unserialize_address_meta(@$row_for_balance_check['address_meta']);
		  ...
		  // Retrieve current balance at address considering required confirmations number and api_timemout value.
			$address_request_array['ctl_payment_id'] = $row_for_balance_check['ctl_payment_id'];
      $address_request_array['ctl_address'] = $row_for_balance_check['ctl_address'];
      $address_request_array['block_index'] = $row_for_balance_check['index_in_wallet'];
		  ...
		  $balance_info_array = ctlWC__getreceivedbyaddress_info($address_request_array, $ctlwc_settings);
        ...
        $current_time = time();
        $query =
          "UPDATE `$ctl_payments_table_name`
             SET
        ...
        if ($balance_info_array['balance'] > 0)
        {
          ...
          ctlWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['ctl_address']}, Payment ID = {$row_for_balance_check['ctl_payment_id']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");

          if ($balance_info_array['balance'] < $last_order_info['order_total'])
          {
            ctlWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: balance at address: '{$row_for_balance_check['ctl_address']}, Payment ID = {$row_for_balance_check['ctl_payment_id']}' (ctl '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
          }
        }
		         ...
		         array (
		            'orders' =>
		               array (
		                  ...
		                  array (
		                     'order_id'     => $order_id,
		                     'order_total'  => $order_total_in_ctl,
	        ...
	        ctlWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['ctl_address']}, Payment ID = {$row_for_balance_check['ctl_payment_id']}' (ctl '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
	        ...
	        ctlWC__process_payment_completed_for_order ($last_order_info['order_id'], $balance_info_array['balance']);
          ...
          //
	        $query =
	          "UPDATE `$ctl_payments_table_name`
	            ...
	            WHERE `id`='$row_id';";
	        $ret_code = $wpdb->query ($query);
	        ctlWC__log_event (__FILE__, __LINE__, "Cron job: SUCCESS: Order ID '{$last_order_info['order_id']}' successfully completed.");

		    }
		  }
		  else
		  {
		    ctlWC__log_event (__FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['ctl_address']} Payment ID: {$row_for_balance_check['ctl_payment_id']}: " . $balance_info_array['message']);
		  }
		  //..//
		}
	}
  ...
  //         ctlWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected LATE PAYMENT non-zero balance at address: '{$row_for_balance_check['ctl_address']}, Payment ID = {$row_for_balance_check['ctl_payment_id']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");
  ...
  //           // Apply exchange rate multiplier only for stores with non-Citadel default currency.
  //           $order_total_in_ctl = $order_total_in_ctl;

  //         $order_total_in_ctl   = sprintf ("%.8f", $order_total_in_ctl);

  //         $difference = $order_total_in_ctl - $last_order_info['order_total'];
  ...
  //                                 __('New Order Total: ', 'woocommerce') . $order_total_in_ctl . ' '.
  //                                 __('Difference: ', 'woocommerce') . $order_total_in_ctl . ' (' . $diffincur . ' '. get_woocommerce_currency() .')'
  //                               );

  //         if ($balance_info_array['balance'] < $last_order_info['order_total'])
  //         {
  //           ctlWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: LATE PAYMENT balance at address: '{$row_for_balance_check['ctl_address']}, Payment ID = {$row_for_balance_check['ctl_payment_id']}' (ctl '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
  //         }
  //       }
  ...
  //         // Last order was fully paid! Complete it...
  //         ctlWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full LATE PAYMENT for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['ctl_address']}, Payment ID = {$row_for_balance_check['ctl_payment_id']}' (ctl '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
  ...
  //             WHERE `id`='$row_id';";
  //         $ret_code = $wpdb->query ($query);
  //         ctlWC__log_event (__FILE__, __LINE__, "Cron job: SUCCESS LATE PAYMENT: Order ID '{$last_order_info['order_id']}' successfully completed.");
  //       }
  //     }
  //     else
  //     {
